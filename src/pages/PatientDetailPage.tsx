...
function generatePatientPDF(patient: ReturnType<typeof getPatient>, dietPlans: DietPlan[], progress: ProgressRecord[]) {
  if (!patient) return;
  ...
  const info = [
    ...
    ['BMI', `${patient.bmi} (${getBMICategory(patient.bmi)})`],
    ['Dosha Type', patient.dosha],
    ['Health Issue', patient.healthIssue || 'None specified'],
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { fontStyle: 'bold', cellWidth: 40, fillColor: [240, 245, 240] },
      1: { cellWidth: 140 },
    },
  ...
  doc.setFontSize(14);
  doc.setTextColor(0);
  doc.text(`${patient.dosha} Diet Guidelines`, 14, y);
  y += 8;

  doc.setFontSize(10);
  doc.setTextColor(34, 87, 60);
  doc.text('Recommended:', 14, y);
  y += 5;
  doc.setTextColor(0);
  rules.recommended.forEach(item => {
    doc.text(`  • ${item}`, 14, y);
    y += 5;
  });
  y += 3;
  ...
  if (dietPlans.length > 0) {
    ...
    doc.setFontSize(14);
    doc.text('Diet Plans', 14, y);
    y += 8;

    autoTable(doc, {
      startY: y,
      head: [['Date', 'Breakfast', 'Lunch', 'Dinner']],
      body: dietPlans.map(p => [p.date, p.breakfast, p.lunch, p.dinner]),
      theme: 'striped',
  ...
  if (progress.length > 0) {
    if (y > 240) { doc.addPage(); y = 20; }
    doc.setFontSize(14);
    doc.text('Progress Records', 14, y);
    y += 8;

    autoTable(doc, {
      startY: y,
      head: [['Date', 'Weight (kg)', 'Remarks']],
      body: progress.map(r => [r.date, r.weight.toString(), r.remarks]),
      theme: 'striped',
      styles: { fontSize: 9, cellPadding: 3 },
      headStyles: { fillColor: [34, 87, 60] },
      margin: { left: 14, right: 14 },
  ...
  doc.save(`${patient.patientId || patient.name}_summary.pdf`);
}
    ...
    <PageTransition>
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-mono text-muted-foreground mb-1">{patient.patientId || 'No ID'}</p>
        ...
        <div className="bg-card border rounded-xl p-6 opacity-0 animate-fade-in-up" style={{ animationDelay: '0.05s' }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <p className="font-mono font-semibold text-foreground">{patient.patientId || 'N/A'}</p>
            </div>
            <div>
            ...
            <div>
              <p className="text-muted-foreground">Diet Plans</p>
              <p className="font-semibold text-foreground">{dietPlans.length}</p>
            </div>
            <div>
              <p className="text-muted-foreground">Progress Entries</p>
              <p className="font-semibold text-foreground">{progress.length}</p>
            </div>
          </div>
          {progress.length > 0 && (
            <div className="mt-4 pt-4 border-t grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="font-semibold text-foreground">{patient.weight} kg</p>
              </div>
              <div>
                <p className="text-muted-foreground">Current Weight</p>
                <p className="font-semibold text-foreground">{progress[progress.length - 1].weight} kg</p>
              </div>
              <div>
                ...
                {(() => {
                  ...
                  return (
                    <p className={`font-semibold ${diff < 0 ? 'text-primary' : diff > 0 ? 'text-destructive' : 'text-foreground'}`}>
